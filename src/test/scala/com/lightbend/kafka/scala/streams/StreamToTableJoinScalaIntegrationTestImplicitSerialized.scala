...
object StreamToTableJoinScalaIntegrationTestImplicitSerialized extends TestSuite[KafkaLocalServer]
  ...
  test("should count clicks per region") { server =>
    ...
    implicit val consumedStringString: Consumed[String, String] = Consumed.`with`(stringSerde, stringSerde)

    val builder = new StreamsBuilderS()

    implicit val consumedStringLong: Consumed[String, Long] = Consumed.`with`(stringSerde, longSerde)
    ...
    implicit val producedStringLong: Produced[String, Long] = Produced.`with`(stringSerde, longSerde)
